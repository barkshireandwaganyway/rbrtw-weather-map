...
# NOTE: there is NO "/data/" in the real path — that was wrong in the original
# version of this script. The server actually serves directly under /2D/.
# Confirmed via: https://mrms.ncep.noaa.gov/2D/MergedReflectivityQCComposite/
MRMS_BASE = 'https://mrms.ncep.noaa.gov/2D'

# mrms.ncep.noaa.gov runs bot-detection that blocks requests with a non-browser
# User-Agent (including generic "RBRTW-Weather-Map/1.0" style strings) — this is
# what caused the 403 Forbidden errors. A realistic browser User-Agent avoids it.
REQUEST_HEADERS = {
...
if __name__ == '__main__':
    ...
    # Wrapped so a misconfigured R2_ENDPOINT produces one clear error here,
    # not a second confusing crash after the per-product failures above.
