...
function setStack(type, title, html) {
  if (!type || !dataEnabled(type)) return;
  state.dataStack[type] = { title: String(title || stackLabels[type] || type), html: String(html || '') };
  renderDataStack();
}
...
function hazardPanel(source, properties = {}, extra = {}) {
  const product = firstValue(properties, ['event', 'headline', 'product', 'outlook', 'label', 'LABEL', 'label2', 'valid', 'phenomena', 'name', 'title'], `${source} Hazard`);
  const risk = firstValue(properties, ['outlook', 'label', 'LABEL', 'label2', 'risk', 'category', 'CATEGORY'], '') || riskLabelFromDn(properties.dn ?? properties.DN ?? properties.gridcode, source);
  const rows = [];
  if (extra.layerName) rows.push(['Layer', extra.layerName]);
  if (risk && risk !== product) rows.push(['Risk / Category', risk]);
  ['severity', 'urgency', 'certainty'].forEach(k => { if (properties[k]) rows.push([k[0].toUpperCase() + k.slice(1), properties[k]]); });
  ...
  const desc = firstValue(properties, ['description', 'snippet', 'summary', 'discussion', 'text'], '');
  if (desc) rows.push(['Details', desc]);
  if (properties.instruction) rows.push(['Action', properties.instruction]);
  ...
  return { title: `${source}: ${sanitizeForPanel(product)}`, html };
}
...
function bindHazardFeature(layer, source, feature, extra = {}) {
  ...
  layer.on('click', async event => {
    ...
    clearProbeMarkers();
    resetDataStack();
    setStack(type, details.title, details.html);
...
async function toggleSpc() {
  ...
  try {
    ...
    setCheck('spcCheck', true); updateLegend('spc'); updatePanel('SPC Outlook', `SPC outlook layer turned on.<br>Polygons loaded: ${features.length}.`);
...
async function toggleWpc() {
  ...
  try {
    ...
    setCheck('wpcCheck', true); updateLegend('wpc'); updatePanel('WPC Outlook', `WPC excessive-rainfall outlook layer turned on.<br>Polygons loaded: ${total}.`);
...
async function runQpfPoint(latlng) {
  ...
  setStack('qpf', 'QPF Forecast Point', `<strong>${sanitizeForPanel(result.layerName)}</strong><br>Forecast liquid precipitation: <strong>${text}</strong><br>Location: ${latlng.lat.toFixed(4)}, ${latlng.lng.toFixed(4)}<br>Checked multiple WPC QPF layers.${matchText}`);
  return true;
}
...
async function runAirQualityPoint(latlng) {
  ...
  setStack('airQuality', 'Air Quality Point', `Category: <strong>${sanitizeForPanel(result.category.label)}</strong><br>PM2.5 value: ${sanitizeForPanel(valueText)}<br>${sanitizeForPanel(result.category.range)}<br>Location: ${latlng.lat.toFixed(4)}, ${latlng.lng.toFixed(4)}.`);
  return true;
}
  ...
  try {
    ...
    for (const result of results) {
      ...
      L.marker([coords[1], coords[0]], { icon: L.divIcon({ className: 'temp-div-icon', html: `<div class="temp-badge ${tempClass(display, mode)}">${Math.round(display)}°</div>`, iconSize: [44, 28], iconAnchor: [22, 14] }) })
        ...
        .on('click', e => { if (e?.originalEvent) L.DomEvent.stopPropagation(e.originalEvent); resetDataStack(); setStack('temp', `Station: ${stationId}`, html); })
...
async function identifySurfaceAt(latlng) {
  ...
  const params = new URLSearchParams({ f: 'json', geometry: `${latlng.lng},${latlng.lat}`, geometryType: 'esriGeometryPoint', sr: '4326', layers: `visible:${(surfaceLayerSets[state.surfaceDay] || surfaceLayerSets[1]).join(',')}`, tolerance: '8', mapExtent: `${b.getWest()},${b.getSouth()},${b.getEast()},${b.getNorth()}`, imageDisplay: `${size.x},${size.y},96`, returnGeometry: 'false' });
...
async function runSurfacePoint(latlng) {
  ...
  if (!results.length) { setStack('surface', 'Surface Map Point', `No WPC surface-map feature returned at selected point.<br>Location: ${latlng.lat.toFixed(4)}, ${latlng.lng.toFixed(4)}`); return true; }
  const rows = results.slice(0, 8).map(result => { const attrs = result.attributes || {}; const label = firstValue(attrs, ['name','Name','LABEL','label','type','TYPE','value','VALUE'], result.layerName || 'Surface feature'); return `<div class="hazard-detail-row"><span>${sanitizeForPanel(result.layerName || 'Layer')}:</span> ${sanitizeForPanel(label)}</div>`; }).join('');
  setStack('surface', 'Surface Map Point', `WPC Day ${state.surfaceDay} National Forecast Chart<br><br>${rows}<br>Location: ${latlng.lat.toFixed(4)}, ${latlng.lng.toFixed(4)}`);
  return true;
}
