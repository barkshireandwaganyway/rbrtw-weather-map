...
function clearLockedHazards(type) {
  ...
  Object.values(locked).forEach(entry => removeLayerSafe(entry.tooltip));
...
function bindHazardFeature(layer, source, feature, extra = {}) {
  ...
  layer.on('click', async event => {
    ...
    if (type === 'spc' || type === 'wpc') {
      ...
      const locked = state.lockedHazards[type];
      if (locked[polyId]) {
        removeLayerSafe(locked[polyId].tooltip);
        ...
        const tooltip = L.tooltip(event.latlng, { permanent: true, direction: 'top', className: 'hazard-tooltip locked-hazard-label', interactive: false })
          .setContent(details.title);
        tooltip.addTo(map);
        locked[polyId] = { tooltip };
...
async function saveMapPhoto() {
  ...
  try {
    ...
    const canvas = await html2canvas(document.body, {
      ...
      scrollY: 0,
      scale,
      ignoreElements: el => (el?.classList?.contains('leaflet-tooltip') && !el?.classList?.contains('locked-hazard-label')) || el?.classList?.contains('leaflet-popup')
    });
