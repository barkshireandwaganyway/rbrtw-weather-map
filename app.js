...
// Guard: prevents the map.on('click') handler from firing immediately after
// a polygon (NWS/SPC/WPC) click and overwriting the polygon's data card.
...
// Case-insensitive property lookup – ArcGIS/SPC/WPC services return UPPERCASE field names
function ciProp(properties, key) {
...
function firstValueCI(properties, keys, fallback = '') {
  for (const key of keys) {
    const v = ciProp(properties, key);
...
function setStack(type, title, html, compact = null) {
  if (!type || !dataEnabled(type)) return;
  state.dataStack[type] = { title: String(title || stackLabels[type] || type), html: String(html || ''), compact: compact ? String(compact) : null };
  renderDataStack();
}

// setStackForce: always writes to data card regardless of the data toggle checkbox.
// Used for direct polygon clicks (NWS alerts, SPC, WPC) so clicking a polygon
// always fills DATA even if the user accidentally unchecked the small data checkbox.
function setStackForce(type, title, html, compact = null) {
  if (!type) return;
  state.dataStack[type] = { title: String(title || stackLabels[type] || type), html: String(html || ''), compact: compact ? String(compact) : null };
  renderDataStack();
}
...
function hazardPanel(source, properties = {}, extra = {}) {
  // Use case-insensitive lookup because ArcGIS/SPC/WPC return UPPERCASE field names
  // (LABEL, VALID, EXPIRE, ISSUE, LABEL2, DN, etc.)
  ...
  const risk = firstValueCI(properties, ['LABEL2', 'label2', 'outlook', 'label', 'LABEL', 'risk', 'category', 'CATEGORY'], '') || riskLabelFromDn(ciProp(properties, 'dn') ?? ciProp(properties, 'gridcode') ?? ciProp(properties, 'DN'), source);
  const rows = [];
  ...
  const riskShortLabel = risk || product || source;
  ['severity', 'urgency', 'certainty'].forEach(k => {
    const v = ciProp(properties, k);
    if (v) rows.push([k[0].toUpperCase() + k.slice(1), String(v)]);
  });
  ...
  const headline = firstValueCI(properties, ['headline'], '');
  if (headline) rows.push(['Headline', headline]);
  const desc = firstValueCI(properties, ['description', 'snippet', 'summary', 'discussion', 'text'], '');
  if (desc) rows.push(['Details', desc.slice(0, 600) + (desc.length > 600 ? '…' : '')]);
  if (properties.instruction) rows.push(['Action', String(properties.instruction).slice(0, 400)]);
  const html = rows.map(([label, value]) => `<div class="hazard-detail-row"><span>${sanitizeForPanel(label)}:</span> ${sanitizeForPanel(value)}</div>`).join('') || 'No detailed properties were returned for this polygon.';

  // Compact version for PNG export — only the most essential fields
  const compactRows = [];
  if (riskShortLabel) compactRows.push(`<strong>${sanitizeForPanel(riskShortLabel)}</strong>`);
  if (valid)   compactRows.push(`Valid: ${sanitizeForPanel(valid)}`);
  if (expires) compactRows.push(`Expires: ${sanitizeForPanel(expires)}`);
  if (headline) compactRows.push(sanitizeForPanel(headline.slice(0, 100)));
  if (area && !headline) compactRows.push(sanitizeForPanel(area.slice(0, 80)));
  const compact = compactRows.join('<br>') || 'See DATA card for details.';
...
function bindHazardFeature(layer, source, feature, extra = {}) {
  ...
  layer.on('click', async event => {
    // Record time so map.on('click') guard can skip the immediately-following map click
    lastPolygonClickTime = Date.now();
    if (event?.originalEvent) L.DomEvent.stopPropagation(event.originalEvent);
    clearProbeMarkers();
    resetDataStack();
    // Always write to DATA card regardless of the small data-card checkbox (setStackForce)
    const locSuffix = event?.latlng
      ? `<br><small>Location: ${event.latlng.lat.toFixed(4)}, ${event.latlng.lng.toFixed(4)}</small>`
      : '';
    setStackForce(type, details.title, details.html + locSuffix, details.compact);
...
async function toggleSpc() {
  if (state.spcLayer) { removeLayerSafe(state.spcLayer); state.spcLayer = null; setCheck('spcCheck', false); clearLegend('spc'); updatePanel('SPC Outlook', 'SPC outlook layer turned off.'); return; }
  try {
    updatePanel('SPC Outlook', 'Loading SPC outlook polygons...');
    const data = await fetchArcGisGeoJson(spcServiceUrl, 1);
    const features = (data.features || []).filter(f => f.geometry);
    state.spcLayer = L.geoJSON({ type: 'FeatureCollection', features }, {
      ...
      onEachFeature: (feature, layer) => bindHazardFeature(layer, 'SPC', feature, { dayLabel: 'Day 1 Convective Outlook' })
    }).addTo(map);
    setCheck('spcCheck', true); updateLegend('spc'); updatePanel('SPC Outlook', `SPC Day 1 Convective Outlook loaded.<br>Polygons: ${features.length}.<br>Click a highlighted polygon to write risk details to the DATA card.`);
  } catch (error) { console.error(error); state.spcLayer = null; setCheck('spcCheck', false); updatePanel('SPC Outlook', 'Could not load SPC outlook polygons.'); }
}
async function toggleWpc() {
  if (state.wpcLayer) { removeLayerSafe(state.wpcLayer); state.wpcLayer = null; setCheck('wpcCheck', false); clearLegend('wpc'); updatePanel('WPC Outlook', 'WPC outlook layer turned off.'); return; }
  try {
    ...
    for (const id of [0, 1, 2, 3, 4]) {
      try {
        ...
        L.geoJSON({ type: 'FeatureCollection', features }, {
          ...
          onEachFeature: (feature, layer) => bindHazardFeature(layer, 'WPC', feature, { layerName: `WPC Excessive Rainfall`, dayLabel: `Layer ${id}` })
        }).addTo(state.wpcLayer);
        total += features.length;
      } catch (layerError) { console.warn('WPC layer failed', id, layerError); }
    }
    setCheck('wpcCheck', true); updateLegend('wpc'); updatePanel('WPC Outlook', `WPC Excessive Rainfall Outlook loaded.<br>Polygons: ${total}.<br>Click a highlighted polygon to write risk details to the DATA card.`);
  } catch (error) { console.error(error); removeLayerSafe(state.wpcLayer); state.wpcLayer = null; setCheck('wpcCheck', false); updatePanel('WPC Outlook', 'Could not load WPC outlook polygons.'); }
}
...
function createQpfLayer() { return L.esri.dynamicMapLayer({ url: qpfServiceUrl, layers: [9], opacity: Number($('qpfOpacity')?.value || 0.65) }); }
function toggleQpf() {
  if (state.qpfLayer) { removeLayerSafe(state.qpfLayer); state.qpfLayer = null; setCheck('qpfCheck', false); clearLegend('qpf'); updatePanel('Rainfall / QPF', 'QPF layer turned off.'); return; }
  state.qpfLayer = createQpfLayer().addTo(map); setCheck('qpfCheck', true); updateLegend('qpf'); updatePanel('Rainfall / QPF', 'WPC QPF 72-hour Day 1–3 layer is on.<br>Click/tap to write point data to the DATA card.');
}
...
async function runQpfPoint(latlng) {
  ...
  const matchText = result.matches?.length ? `<br>Matched layers: ${result.matches.map(m => `${sanitizeForPanel(m.layerName)} ${formatInches(m.value)}`).join('; ')}` : '';
  const compact = `72-hr QPF: <strong>${text}</strong>`;
  setStack('qpf', 'QPF Forecast Point', `<strong>${sanitizeForPanel(result.layerName)}</strong><br>Forecast liquid precipitation: <strong>${text}</strong><br>Location: ${latlng.lat.toFixed(4)}, ${latlng.lng.toFixed(4)}<br>Checked multiple WPC QPF layers.${matchText}`, compact);
  return true;
}
...
async function identifyRainfallAt(latlng) {
  ...
  // Always check 4 cardinal nearby points (~2.5km offset) because MRMS raster
  // resolution (~1km) can make the exact clicked pixel underrepresent the
  // surrounding rainfall event. This is most visible in high-rain areas where
  // a single pixel edge sits between color bands.
  const offset = 0.022; // ~2.5km
  const nearbyPts = [
  ...
  if (!valid.length) return { ...exact, nearbyChecked: nearbyPts.length, usedNearby: false, exactPointInches: asNumber(exact.inches) };
  ...
  // Use the nearby maximum when it's more than 0.15 in higher than the exact point
  // (threshold avoids inflating values near zero-coverage areas)
  if (best !== exact && bestIn > exactIn + 0.15) {
    return { ...best, nearbyChecked: nearbyPts.length, usedNearby: true, exactPointInches: exactIn };
  }
  return { ...exact, nearbyChecked: nearbyPts.length, usedNearby: false, exactPointInches: exactIn };
}
async function runRainfallPoint(latlng) {
  ...
  const unitNote = rawMm !== null && rawMm > 25 ? `<br>Raw raster value: ${rawMm.toFixed(1)} mm → converted to inches.` : `<br>Raw sample: ${sanitizeForPanel(result.raw ?? 'N/A')}`;
  const sourceNote = result.usedNearby
    ? `<br><strong>Nearby area max used.</strong> Exact clicked point returned: ${formatInches(result.exactPointInches)}.<br>MRMS raster resolution can cause a single pixel to sample lower than the surrounding event.`
    : `<br>Sampled from exact clicked point.`;
  const fullHtml = `<strong>${rainfallLabel()}</strong><br>Estimated observed rainfall total: <strong>${text}</strong>${unitNote}${sourceNote}<br>Location: ${latlng.lat.toFixed(4)}, ${latlng.lng.toFixed(4)}<br>Source: NOAA/NWS MRMS QPE (${sanitizeForPanel(result.source || 'sample')}).`;
  const compact = `${rainfallLabel()}: <strong>${text}</strong>${result.usedNearby ? '<br><em>Area nearby max used</em>' : ''}`;
  setStack('rainfall', 'Rainfall Total Point', fullHtml, compact);
  return true;
}
...
async function runAirQualityPoint(latlng) {
  ...
  setStack('airQuality', 'Air Quality Point', `Category: <strong>${sanitizeForPanel(result.category.label)}</strong><br>PM2.5 value: ${sanitizeForPanel(valueText)}<br>${sanitizeForPanel(result.category.range)}<br>Location: ${latlng.lat.toFixed(4)}, ${latlng.lng.toFixed(4)}.`, `AQ: <strong>${sanitizeForPanel(result.category.label)}</strong> — ${sanitizeForPanel(valueText)}`);
  return true;
}
...
function buildTempCompactHtml(stationName, stationId, p, calculated, windMph, gustMph, dewF, humidity) {
  // Compact version for PNG export — only priority fields
  const lines = [`<strong>${sanitizeForPanel(stationName || stationId)}</strong>`];
  lines.push(`Temp: ${Math.round(calculated.tempF)}°F`);
  if (dewF !== null) lines.push(`Dew Pt: ${Math.round(dewF)}°F`);
  if (humidity !== null) lines.push(`Humidity: ${Math.round(humidity)}%`);
  if (calculated.heatIndex !== null) lines.push(`Heat Index: ${Math.round(calculated.heatIndex)}°F`);
  if (calculated.windChill !== null) lines.push(`Wind Chill: ${Math.round(calculated.windChill)}°F`);
  if (windMph !== null) lines.push(`Wind: ${Math.round(windMph)} mph${gustMph !== null ? ` / Gust: ${Math.round(gustMph)} mph` : ''}`);
  if (p.timestamp) lines.push(`Updated: ${new Date(p.timestamp).toLocaleTimeString()}`);
...
async function toggleTemperatures() {
  ...
  try {
    ...
    for (const result of results) {
      ...
      const tempF = cToF(p.temperature?.value); if (tempF === null) continue; const humidity = asNumber(p.relativeHumidity?.value); const windMph = mpsToMph(p.windSpeed?.value);
      ...
      let display = tempF, labelType = 'Temp', mode = 'temp'; if (state.tempDisplayMode === 'heat' && hi !== null) { display = hi; labelType = 'Heat Index'; mode = 'heat'; } if (state.tempDisplayMode === 'windchill' && wc !== null) { display = wc; labelType = 'Wind Chill'; mode = 'windchill'; }
      const stationId = station.properties?.stationIdentifier || station.id.split('/').pop(); const html = buildStationHtml(station, obs, { tempF, heatIndex: hi, windChill: wc });
      const compact = buildTempCompactHtml(station.properties?.name || stationId, stationId, p, { tempF, heatIndex: hi, windChill: wc }, windMph, gustMph, dewF, humidity);
...
function createSurfaceLayer(day = state.surfaceDay) {
  // Do not specify 'layers' so the service uses its own default visible layers.
  // Specifying layer IDs that no longer exist in the service returns a blank image.
...
function toggleSurfaceMap() {
  ...
  updateLegend('surface');
  updatePanel('Surface Map', `WPC National Forecast Chart is on (Day ${state.surfaceDay} selected).<br>Use Day 1, Day 2, or Day 3 to record your selection. Click/tap visible features to write details to the DATA card.`);
}
async function identifySurfaceAt(latlng) {
  const size = map.getSize(); const b = map.getBounds();
  // Use 'all' layers since we don't specify layer IDs in the map layer.
  // Increase tolerance so lines and point features are also identifiable.
  const params = new URLSearchParams({
    f: 'json',
...
async function runSurfacePoint(latlng) {
  ...
  if (!results.length) {
    setStack('surface', 'Surface Map Point', `Day ${state.surfaceDay} selected.<br>No WPC surface-map feature returned at this point.<br>Try clicking directly on a visible front, high, or low.<br>Location: ${latlng.lat.toFixed(4)}, ${latlng.lng.toFixed(4)}`);
    return true;
  }
  const rows = results.slice(0, 10).map(result => {
    const attrs = result.attributes || {};
    const label = firstValueCI(attrs, ['name','Name','LABEL','label','type','TYPE','value','VALUE','featureType','FeatureType'], result.layerName || 'Surface feature');
  ...
  const compact = results.slice(0, 3).map(r => {
    ...
    const lbl = firstValueCI(attrs, ['name','LABEL','label','type','TYPE'], r.layerName || 'Feature');
    return sanitizeForPanel(`${r.layerName || ''}: ${lbl}`);
  }).join('<br>');
  setStack('surface', 'Surface Map Point', `WPC Day ${state.surfaceDay} National Forecast Chart<br>${rows}<br>Location: ${latlng.lat.toFixed(4)}, ${latlng.lng.toFixed(4)}`, compact || `Day ${state.surfaceDay} — click on visible feature`);
  return true;
}
...
map.on('click', async event => {
  // Guard: if a polygon (NWS/SPC/WPC) was just clicked, the same DOM click
  // event bubbles to the map and would overwrite the polygon's data card.
...
// Builds a condensed DATA card for PNG export.
// Uses per-type compact HTML stored in state.dataStack[type].compact when available,
// falling back to a stripped version of the full HTML.
function buildExportDataHtml() {
  ...
  const sectionHtml = types.map(type => {
    const item = state.dataStack[type];
    // Prefer pre-built compact HTML; otherwise strip the full HTML
    const body = item.compact || stripToExportCompact(item.html, type);
    return `<div class="stack-data-section stack-data-${type}" style="margin:3px 0;padding:5px 8px;max-height:none;">
      <div class="stack-data-title" style="font-size:9.5px;">${sanitizeForPanel(stackLabels[type] || type)}</div>
      <div class="stack-data-subtitle" style="font-size:9.5px;margin-bottom:3px;">${sanitizeForPanel(item.title)}</div>
      <div class="stack-data-body" style="font-size:10px;line-height:1.22;">${body}</div>
    </div>`;
  }).join('');

  return `<div class="stack-data-card-title" style="font-size:12px;margin-bottom:3px;">MAP CLICK DATA</div>${sectionHtml}`;
}

function stripToExportCompact(html, type) {
  if (!html) return '';
  let s = html
    // Remove the large temperature badge
    .replace(/<div class="big-temp"[^>]*>.*?<\/div>/gi, '')
    // Remove section headers (Wind, Pressure / Visibility, etc.)
    .replace(/<div class="data-section-title">.*?<\/div>/gi, '')
    // Collapse excess line breaks
    .replace(/(<br\s*\/?>\s*){3,}/gi, '<br>');
  // For temperature, strip after the wind rows to keep it short
  if (type === 'temp') {
    const windCutIdx = s.toLowerCase().indexOf('pressure');
    if (windCutIdx > 80) s = s.slice(0, windCutIdx);
  }
  // For NWS/SPC/WPC alerts, strip long description text
  if (['alerts','spc','wpc'].includes(type)) {
    const detailIdx = s.indexOf('Details:');
    if (detailIdx > 50) s = s.slice(0, detailIdx) + '<em>…see live DATA card for full text</em>';
  }
...
async function saveMapPhoto() {
  ...
  try {
    ...
    if (includeData) {
      // Build condensed export HTML from the dataStack (not from the live DOM)
