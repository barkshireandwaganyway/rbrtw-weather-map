...
function _buildRegistryRow(layer) {
  const statusBadge = {
    ...
    placeholder:  '<span class="adv-badge placeholder">COMING SOON</span>',
    disabled:     '<span class="adv-badge disabled">UNAVAILABLE</span>',
  }[layer.status] || '';

  // A layer is usable if it's active/experimental, OR if it has a dataUrl
  // that will resolve once BACKEND_BASE_URL is set.
  const hasBackend = !!(layer.dataUrl);
  const isUsable   = layer.status === 'active' || layer.status === 'experimental' || hasBackend;
  const disabledAttr = isUsable ? '' : 'disabled';
  const dim          = isUsable ? '' : ' style="opacity:0.45"';

  // Checkbox: wired to toggleRegistryLayer for backend layers, disabled otherwise
  ...
  let noteHtml = '';
  if (!isUsable) {
    const brief = layer.status === 'disabled'
      ? 'No public source — license restriction or not publicly available.'
      : 'Coming soon — requires server-side processing to display in browser.';
    noteHtml = `<div class="adv-row-brief">${brief}</div>`;
  } else if (hasBackend && !BACKEND_BASE_URL) {
    noteHtml = `<div class="adv-row-brief">Set BACKEND_BASE_URL in app.js to enable.</div>`;
  }

  return `<div class="adv-layer-entry"${dim}>
  <div class="adv-layer-main">
    <input type="checkbox" ${disabledAttr} ${cbExtra}/>
