...
# NOTE: if this script starts failing with 403 Forbidden (the same
# bot-detection issue that hit mrms.ncep.noaa.gov), NOAA also publishes RAP
# and GFS to public S3 buckets built for exactly this kind of automated
# access: s3://noaa-rap-pds and s3://noaa-gfs-bdp-pds (both unauthenticated,
# same boto3 + UNSIGNED config pattern used in render_mrms.py). Switch to
# those if NOMADS ever blocks GitHub Actions the way the MRMS site did.
