...
// Dedicated pane for satellite imagery (GOES, backend-rendered registry layers).
// Sits ABOVE the base tile pane (z=200) but BELOW the default overlayPane (z=400)
// where surface fronts, radar, hazard polygons, and markers live. This guarantees
// those layers always stay visible on top of satellite imagery, no matter which
// order the user toggles things on — previously, whichever layer was toggled on
// most recently won the stacking order, so satellite could hide the surface map.
map.createPane('satOverlayPane');
map.getPane('satOverlayPane').style.zIndex = 250;
map.getPane('satOverlayPane').style.pointerEvents = 'none';

// Dedicated pane for the WPC Surface Map so a CSS filter can boost its
// perceived solidity. The surface map is a server-rendered PNG from NOAA —
// our opacity slider can only dim/brighten that image, it cannot make its
// colors more saturated or solid than however NOAA's own renderer drew
// them. Boosting saturation/contrast on this pane is the one thing we
// genuinely can do client-side to make it stand out more, especially
// against busy basemaps like satellite imagery.
map.createPane('surfaceOverlayPane');
map.getPane('surfaceOverlayPane').style.zIndex = 260;
...
const state = {
  ...
  rbrtwMarker: null,
  rbrtwCircle: null,
  rbrtwPosition: [RBRTW_AREA[0], RBRTW_AREA[1]],
  rbrtwVisible: true,
  lockedHazards: { spc: {}, wpc: {} },
  polyIdCounter: 0,
  radarLayer: null,
  pastRadarLayer: null,
  radarFrames: [],
  ...
  tempScope: 'local',    // 'local' = stations near RBRTW pin | 'national' = CONUS ASOS list
  windScope: 'local',
  goesLayer: null,
...
// ---------------------------------------------------------------------------
// National station scope — curated list of major ASOS stations across CONUS,
// used when Temperatures or Wind Barbs is set to "National" mode.
// The NWS API /stations endpoint has no bbox filter, so we maintain this
// curated list (~80 stations evenly spread across CONUS) rather than querying
// all 10,000+ stations. Observations are fetched in parallel and then filtered
// to only render the ones currently visible on the map.
// ---------------------------------------------------------------------------
const NATIONAL_ASOS_STATIONS = [
  // Pacific NW / Rockies
  'KSEA','KPDX','KBOI','KBZN','KGTF','KBIL','KCOE',
  // CA / NV / AZ
  'KLAX','KSFO','KPHX','KLAS','KRNO','KFAT','KSJC','KSAN',
  // N. Plains
  'KFAR','KBIS','KABR','KRAP','KCYS','KGRI','KOMA',
  // S. Plains / TX
  'KABQ','KELP','KMAF','KAMA','KLBB','KSPS','KICT','KTUL',
  'KLAW','KABI','KSJT','KSAT','KDFW','KHOU','KAUS','KCLL',
  'KBRO','KCRP','KIAH','KACT',
  // Great Lakes / Midwest
  'KMSP','KDLH','KGRB','KMKE','KORD','KDET','KGRR','KCMH',
  // Deep South
  'KSTL','KMEM','KJAN','KLIT','KSHV','KMSY','KGSP',
  'KBHM','KMOB','KHSV','KPNS',
  // SE / FL / GA / SC
  'KTPA','KMIA','KJAX','KMCO','KATL','KCHA','KCHS','KSAV',
  // Mid-Atlantic / NE
  'KBNA','KBWI','KDCA','KIAD','KJFK','KPHL','KPIT','KBOS',
  'KBUF','KALB','KSYR',
  // New England
  'KBTV','KCON','KPWM','KBGR',
];
...
// Reverse-geocode a lat/lon into a short "City, ST" label for the DATA card,
// instead of showing raw coordinates. Cached by rounded coordinate (~1km grid)
// to avoid repeat lookups for nearby clicks. Falls back to coordinates if the
// lookup fails or is slow — the user always sees something useful.
// Note: uses OSM Nominatim (nominatim.openstreetmap.org), a free public service
// with a ~1 request/sec usage policy — fine for personal/low-traffic use.
const _geoLabelCache = new Map();
async function locationLabel(lat, lon) {
  const key = `${lat.toFixed(2)},${lon.toFixed(2)}`;
  if (_geoLabelCache.has(key)) return _geoLabelCache.get(key);
  ...
  try {
    ...
    const label = place ? `${place}${state ? ', ' + state : ''}` : coordFallback;
    _geoLabelCache.set(key, label);
    return label;
  } catch (_) {
    clearTimeout(timer);
    return coordFallback; // network error, timeout, or no result — show coordinates
  }
}
...
function resetDataStack() {
  // Locked SPC/WPC entries (from clicking a polygon to "pin" it) persist across
  // every other click — they only go away if you click that same polygon again,
  // or the underlying SPC/WPC layer itself is turned off / reloaded for a new day.
  const preservedSpc = state.dataStack.spc;
  const preservedWpc = state.dataStack.wpc;
  state.dataStack = {};
  if (preservedSpc) state.dataStack.spc = preservedSpc;
  if (preservedWpc) state.dataStack.wpc = preservedWpc;
  state.clickRunId++;
}
...
// ----- Pinned SPC/WPC stack (multiple polygons can be pinned at once) -----
// Adds one polygon's data as a new entry in state.dataStack[type], which for
// 'spc'/'wpc' is an array rather than a single object — each pinned polygon
// gets its own DATA card section, newest pin on top.
function addStackEntry(type, id, title, html, compact) {
  if (!Array.isArray(state.dataStack[type])) state.dataStack[type] = [];
  state.dataStack[type] = state.dataStack[type].filter(e => e.id !== id);
  state.dataStack[type].unshift({ id, title: String(title || ''), html: String(html || ''), compact: compact != null ? String(compact) : null });
  renderDataStack();
}
function removeStackEntry(type, id) {
  if (!Array.isArray(state.dataStack[type])) return;
  state.dataStack[type] = state.dataStack[type].filter(e => e.id !== id);
  renderDataStack();
}
// Called when the SPC/WPC layer itself is turned off or reloaded for a new
// day — the old polygon layer objects are gone, so their pinned labels and
// IDs are no longer meaningful and must be cleared to avoid orphaned labels.
function clearLockedHazards(type) {
  const locked = state.lockedHazards[type] || {};
  Object.values(locked).forEach(entry => {
    if (entry.reposition) map.off('move zoom', entry.reposition);
    if (entry.labelEl) entry.labelEl.remove();
  });
  state.lockedHazards[type] = {};
  state.dataStack[type] = [];
}

function renderDataStack() {
  const status = $('status');
  if (!status) return;
  const types = stackOrder.filter(type => {
    const entry = state.dataStack[type];
  ...
  status.innerHTML = `
    ...
    <div class="stack-data-sections">
      ${types.map(type => {
        ...
        return items.map(item => `<div class="stack-data-section stack-data-${type}">
...
function showRbrtwCircle() {
  ...
  state.rbrtwCircle = L.circle(state.rbrtwPosition, { radius: 9000, color: '#ff3b3b', weight: 1, fillColor: '#ff3b3b', fillOpacity: 0.12 }).addTo(map);
}

function focusArea() {
  map.setView(state.rbrtwPosition, 11);
  showRbrtwCircle();
  if (state.rbrtwMarker) state.rbrtwMarker.openTooltip();
}

function initRbrtwMarker() {
  state.rbrtwMarker = L.marker(state.rbrtwPosition, { draggable: true })
    .addTo(map)
    .bindTooltip('RBRTW AREA (drag to move)', { permanent: false, direction: 'top', className: 'hazard-tooltip' });
  state.rbrtwMarker.on('dragend', async e => {
    const pos = e.target.getLatLng();
    state.rbrtwPosition = [pos.lat, pos.lng];
    if (state.rbrtwCircle) state.rbrtwCircle.setLatLng(pos);
    // Re-fetch NWS office/grid + forecast data for the new pin location
    updatePanel('RBRTW AREA', 'Pin moved — loading data for new location…');
    await loadNwsPointData();
  });
  showRbrtwCircle();
}

// Toggle the RBRTW pin + accuracy circle on/off entirely
function toggleRbrtwPin() {
  state.rbrtwVisible = !state.rbrtwVisible;
  if (state.rbrtwMarker) { if (state.rbrtwVisible) state.rbrtwMarker.addTo(map); else map.removeLayer(state.rbrtwMarker); }
  if (state.rbrtwCircle) { if (state.rbrtwVisible) state.rbrtwCircle.addTo(map); else map.removeLayer(state.rbrtwCircle); }
  const btn = $('rbrtwPinToggleBtn');
  if (btn) btn.classList.toggle('pin-hidden', !state.rbrtwVisible);
}

// Opacity slider for the pin/circle ("transparent bar")
function setRbrtwOpacity(value) {
  const v = Number(value);
  if (state.rbrtwMarker?.setOpacity) state.rbrtwMarker.setOpacity(v);
  if (state.rbrtwCircle?.setStyle) state.rbrtwCircle.setStyle({ opacity: v, fillOpacity: v * 0.12 });
}

function getNwsPointData() {
  const [lat, lon] = state.rbrtwPosition;
  return fetch(`https://api.weather.gov/points/${lat},${lon}`, { headers: { Accept: 'application/geo+json' } })
...
function surfaceSymbolRow(svgHtml, label, value = '') {
  return `<div class="mapkey-row mapkey-surface-row">
    <span class="mapkey-surface-symbol">${svgHtml}</span>
    <span class="mapkey-label">${label}</span>
    <span class="mapkey-value">${value}</span>
  </div>`;
}
function surfaceKeyHtml() {
  // SVG weather front/symbol definitions drawn to match standard WPC notation
  const coldFront = `<svg width="38" height="14" viewBox="0 0 38 14">
    <line x1="2" y1="7" x2="36" y2="7" stroke="#1683ff" stroke-width="2.5"/>
    <polygon points="10,7 4,2 4,12" fill="#1683ff"/>
    <polygon points="22,7 16,2 16,12" fill="#1683ff"/>
    <polygon points="34,7 28,2 28,12" fill="#1683ff"/>
  </svg>`;
  const warmFront = `<svg width="38" height="14" viewBox="0 0 38 14">
    <line x1="2" y1="7" x2="36" y2="7" stroke="#ff3434" stroke-width="2.5"/>
    <circle cx="9"  cy="7" r="5" fill="none" stroke="#ff3434" stroke-width="2.5" clip-path="url(#top)"/>
    <circle cx="21" cy="7" r="5" fill="none" stroke="#ff3434" stroke-width="2.5" clip-path="url(#top)"/>
    <circle cx="33" cy="7" r="5" fill="none" stroke="#ff3434" stroke-width="2.5" clip-path="url(#top)"/>
    <defs><clipPath id="top"><rect x="0" y="0" width="38" height="7"/></clipPath></defs>
  </svg>`;
  const stationaryFront = `<svg width="38" height="14" viewBox="0 0 38 14">
    <line x1="2" y1="7" x2="36" y2="7" stroke="#7c3aed" stroke-width="2.5"/>
    <polygon points="9,7 3,2 3,12" fill="#1683ff"/>
    <circle cx="21" cy="7" r="5" fill="none" stroke="#ff3434" stroke-width="2.5" clip-path="url(#bot)"/>
    <polygon points="33,7 27,2 27,12" fill="#1683ff"/>
    <defs><clipPath id="bot"><rect x="0" y="7" width="38" height="7"/></clipPath></defs>
  </svg>`;
  const trough = `<svg width="38" height="14" viewBox="0 0 38 14">
    <line x1="2" y1="7" x2="36" y2="7" stroke="#8b5a2b" stroke-width="2.5" stroke-dasharray="5,3"/>
  </svg>`;
  const highLow = `<svg width="38" height="14" viewBox="0 0 38 14">
    <text x="7" y="11" font-family="Arial,sans-serif" font-size="12" font-weight="900" fill="#1a56db">H</text>
    <text x="22" y="11" font-family="Arial,sans-serif" font-size="12" font-weight="900" fill="#dc2626">L</text>
  </svg>`;
  return [
    surfaceSymbolRow(highLow,          'Highs / Lows',           'Pressure centers'),
    surfaceSymbolRow(coldFront,        'Cold Front',             'Boundary'),
    surfaceSymbolRow(warmFront,        'Warm Front',             'Boundary'),
    surfaceSymbolRow(stationaryFront,  'Stationary / Occluded',  'Boundary'),
    surfaceSymbolRow(trough,           'Trough / Dryline',       'Boundary'),
    keyRow('#7dd3fc', 'Rain / Thunderstorms', 'WPC'),
    keyRow('#ef4444', 'Severe Possible',      'Hazard area'),
    keyRow('#22c55e', 'Heavy Rain / Flood',   'Hazard area'),
    keyRow('#f97316', 'Critical Fire Weather','Hazard area'),
    keyRow('#93c5fd', 'Freezing Rain',        'Hazard area'),
    keyRow('#e0f2fe', 'Heavy Snow',           'Hazard area'),
  ].join('');
}
...
function bindHazardFeature(layer, source, feature, extra = {}) {
  ...
  // Stable ID for this specific polygon's lifetime (until its layer is removed/reloaded)
  const polyId = ++state.polyIdCounter;
  layer._rbrtwPolyId = polyId;
  ...
  layer.on('click', async event => {
    ...
    if (type === 'spc' || type === 'wpc') {
      // Pin/unpin toggle: clicking this polygon a 2nd time removes its pinned
      // label + DATA card entry. Clicking a DIFFERENT polygon adds a new one
      // alongside any already pinned — none of them get wiped by other clicks.
      const locked = state.lockedHazards[type];
      if (locked[polyId]) {
        map.off('move zoom', locked[polyId].reposition);
        locked[polyId].labelEl.remove();
        delete locked[polyId];
        removeStackEntry(type, polyId);
      } else {
        const locNote = event?.latlng ? `<br><small>Location: ${sanitizeForPanel(await locationLabel(event.latlng.lat, event.latlng.lng))}</small>` : '';
        // Plain self-positioned div instead of a Leaflet Tooltip — Leaflet's
        // own Tooltip class (especially a "permanent" one added directly to
        // the map rather than bound to a layer) renders fine live but does
        // not reliably survive html2canvas's clone-based export capture.
        // A plain div positioned via latLngToContainerPoint uses the same
        // technique already proven to export correctly for the DATA card.
        const labelEl = document.createElement('div');
        labelEl.className = 'hazard-tooltip locked-hazard-label';
        labelEl.style.position = 'absolute';
        labelEl.style.transform = 'translate(-50%, -100%)';
        labelEl.style.pointerEvents = 'none';
        labelEl.style.zIndex = '700';
        labelEl.textContent = details.title;
        const reposition = () => {
          const p = map.latLngToContainerPoint(event.latlng);
        ...
        locked[polyId] = { labelEl, reposition };
        addStackEntry(type, polyId, details.title, details.html + locNote, details.compact);
      }
      if (event?.latlng) await runAllCheckedPointData(event.latlng, [type]);
      return;
    }

    // Existing single-slot behavior (alerts, etc.) — unchanged
    resetDataStack();
    const locNote = event?.latlng ? `<br><small>Location: ${sanitizeForPanel(await locationLabel(event.latlng.lat, event.latlng.lng))}</small>` : '';
...
function setSpcDay(day) {
  ...
  // Reload layer for new day — old polygons (and any pinned labels on them) are gone
  removeLayerSafe(state.spcLayer); state.spcLayer = null;
  clearLockedHazards('spc'); renderDataStack();
  _loadSpcLayer();
}
...
function setWpcDay(day) {
  ...
  clearLockedHazards('wpc'); renderDataStack();
  _loadWpcLayer();
}
...
async function runPastRadarPoint(latlng) {
  if (!state.pastRadarLayer || !dataEnabled('pastRadar')) return false;
  const frame = state.radarFrames[state.radarIndex] || {};
  const loc = await locationLabel(latlng.lat, latlng.lng);
  setStack('pastRadar', 'Past Radar Point', `Frame: <strong>${frame.time ? sanitizeForPanel(localRadarTime(frame.time)) : 'Latest'}</strong><br>Location: ${sanitizeForPanel(loc)}<br>RainViewer playback tiles do not expose pixel dBZ sampling.`);
  return true;
}
...
async function runQpfPoint(latlng) {
  ...
  const loc = await locationLabel(latlng.lat, latlng.lng);
  setStack('qpf', 'QPF Forecast Point', `<strong>${sanitizeForPanel(result.layerName)}</strong><br>Forecast precipitation: <strong>${text}</strong><br>Location: ${sanitizeForPanel(loc)}${matchText}`, compact);
  return true;
}
...
async function runRainfallPoint(latlng) {
  ...
  setStack('rainfall', 'Rainfall Total Point',
    `<strong>${rainfallLabel()}</strong><br>MRMS QPE estimated total: <strong>${text}</strong>${unitNote}${srcNote}<br>Location: ${sanitizeForPanel(loc)}<br>Source: NOAA/NWS MRMS QPE (${sanitizeForPanel(result.source || 'sample')}).`,
    compact);
  return true;
}
...
async function runAirQualityPoint(latlng) {
  ...
  setStack('airQuality', 'Air Quality Point',
    `Category: <strong>${sanitizeForPanel(result.category.label)}</strong><br>PM2.5 value: ${sanitizeForPanel(valueText)}<br>${sanitizeForPanel(result.category.range)}<br>Location: ${sanitizeForPanel(loc)}.`,
...
// Fetch latest observations from the national curated ASOS list in parallel,
// returning only stations with valid data that are currently visible on the map.
// This is how "National" mode covers CONUS without a bbox API endpoint.
async function fetchNationalStationObs() {
  const bounds = map.getBounds();
  const results = await Promise.allSettled(
    NATIONAL_ASOS_STATIONS.map(id =>
      fetch(`https://api.weather.gov/stations/${id}/observations/latest`,
        { headers: { Accept: 'application/geo+json', 'User-Agent': 'rbrtw-weather-map' } })
        .then(r => { if (!r.ok) throw new Error('obs failed'); return r.json(); })
        .then(json => ({ id, obs: json }))
    )
  );
  return results
...
// National stations don't come with a pre-built station metadata object the
// way the local flow does, so we build a minimal compatible one from the obs.
function stationFromObs(id, obs) {
  const coords = obs.geometry?.coordinates;
  return {
    id: `https://api.weather.gov/stations/${id}`,
    geometry: { coordinates: coords },
    properties: { stationIdentifier: id, name: obs.properties?.station?.split('/').pop() || id }
  };
}

function setTempScope(scope) {
  state.tempScope = scope;
  $('tempScopeLocal')?.classList.toggle('scope-active', scope === 'local');
  $('tempScopeNational')?.classList.toggle('scope-active', scope === 'national');
  if (state.tempLayer) {
    removeLayerSafe(state.tempLayer); state.tempLayer = null;
    setCheck('tempCheck', false);
    toggleTemperatures();
  }
}

function setWindScope(scope) {
  state.windScope = scope;
  $('windScopeLocal')?.classList.toggle('scope-active', scope === 'local');
  $('windScopeNational')?.classList.toggle('scope-active', scope === 'national');
  if (state.windLayer) {
    removeLayerSafe(state.windLayer); state.windLayer = null;
...
async function toggleTemperatures() {
  ...
  try {
    ...
    let stationObsPairs = [];
    if (state.tempScope === 'national') {
      updatePanel('Temperatures', 'Loading national temperature stations…');
      const natResults = await fetchNationalStationObs();
      stationObsPairs = natResults.map(({ id, obs }) => ({ station: stationFromObs(id, obs), obs }));
    } else {
      const point = await getNwsPointData();
      const stationsData = await fetch(point.properties.observationStations).then(r => { if (!r.ok) throw new Error('Stations failed'); return r.json(); });
      ...
      stationObsPairs = results.filter(r => r.status === 'fulfilled').map(r => r.value);
    }
    let plotted = 0;
    for (const { station, obs } of stationObsPairs) {
...
async function toggleWindLayer() {
  ...
  try {
    ...
    let stationObsPairs = [];
    if (state.windScope === 'national') {
      updatePanel('Wind Barbs', 'Loading national wind stations…');
      const natResults = await fetchNationalStationObs();
      stationObsPairs = natResults.map(({ id, obs }) => ({ station: stationFromObs(id, obs), obs }));
    } else {
      const point = await getNwsPointData();
    ...
    for (const { station, obs } of stationObsPairs) {
      const coords = station.geometry?.coordinates; const p = obs.properties || {}; if (!coords) continue;
    ...
    setCheck('windCheck', true); updateLegend('wind'); updatePanel('Wind Barbs', `${state.windScope === 'national' ? 'National' : 'Local'} wind stations loaded.<br>Stations plotted: ${plotted}.<br>Arrow points where the wind is moving.`);
...
// 4-option county line color — lets the user pick a color that's visible
// against whichever basemap (light/dark/satellite) is currently active.
const COUNTY_COLORS = { white: '#ffffff', black: '#111111', yellow: '#ffd400', cyan: '#22d3ee' };
function countyName(feature) { const p = feature?.properties || {}; return p.NAME || p.name || p.NAMELSAD || p.COUNTY || p.COUNTY_NAME || 'County'; }
function countyStyle() { return { color: COUNTY_COLORS[state.countyColor] || '#ffffff', weight: 1.5, opacity: Number($('countyOpacity')?.value || 0.25), fillOpacity: 0, interactive: true }; }
function setCountyColor(key) {
  state.countyColor = COUNTY_COLORS[key] ? key : 'white';
  document.querySelectorAll('.county-color-swatch').forEach(el => el.classList.toggle('active-swatch', el.dataset.color === state.countyColor));
...
async function toggleCountyLines() {
  ...
  try {
    ...
    state.countyLayer = L.geoJSON(data, {
      // renderer: L.canvas() is REQUIRED for correct PNG export alignment.
      // Leaflet's default SVG renderer applies a CSS transform to the SVG pane
      // that html2canvas does not honour, making county lines appear at the
      // wrong geographic position (often shifted toward a corner) in exported
      // PNGs. Canvas renderer draws at real pixel coordinates so html2canvas
      // captures it correctly every time.
      renderer: L.canvas(),
      style: countyStyle,
      onEachFeature: (feature, layer) => { const name = countyName(feature); layer.bindTooltip(`${sanitizeForPanel(name)} County`, { sticky: true, direction: 'top' }); layer.on('click', e => { if (e?.originalEvent) L.DomEvent.stopPropagation(e.originalEvent); resetDataStack(); setStack('county', 'County Boundary', `${sanitizeForPanel(name)} County<br>Layer: Texas county boundary lines`); }); }
    }).addTo(map);
...
function createSurfaceLayer(day = state.surfaceDay) {
  ...
  return L.esri.dynamicMapLayer({ url: surfaceMapServiceUrl, opacity: Number($('surfaceOpacity')?.value || 0.78), pane: 'surfaceOverlayPane' });
}
// The WPC National Forecast Chart service does not render reliably when the
// requested image covers a very wide area (e.g. zoomed out to see most/all
// of the CONUS) — the request appears to fail silently, and toggling the
// layer off/on again doesn't help because the same oversized request just
// fails the same way every time. The only thing that worked was reloading
// the whole page and zooming in BEFORE turning the layer on — i.e. the fix
// is to never send that oversized request in the first place, not to clear
// some stuck internal state (there wasn't any).
const MIN_SURFACE_ZOOM = 5;
...
function toggleSurfaceMap() {
  ...
  if (map.getZoom() < MIN_SURFACE_ZOOM) {
    setCheck('surfaceCheck', false);
    updatePanel('Surface Map',
      `Zoom in a bit further before turning on the Surface Map.<br>This NOAA/WPC service does not render reliably at very wide (near-nationwide) zoom levels — zoom in to at least a multi-state view, then try again.`);
    return;
  }
...
// If the user zooms out too far WHILE the surface layer is on, turn it off
// automatically with a clear explanation, rather than leaving it stuck
// showing a blank/failed render. If they zoom back in, nudge it to redraw.
map.on('zoomend', () => {
  ...
  if (map.getZoom() < MIN_SURFACE_ZOOM) {
    ...
    setCheck('surfaceCheck', false);
    clearLegend('surface');
    updatePanel('Surface Map', 'Surface map turned off — zoomed out too far for this NOAA/WPC service to render reliably. Zoom in and turn it back on.');
  } else if (typeof state.surfaceLayer.redraw === 'function') {
    state.surfaceLayer.redraw();
  }
});
...
async function runSurfacePoint(latlng) {
  ...
  if (!results.length) {
    setStackForce('surface', 'Surface Map Point',
      `Day ${state.surfaceDay} selected.<br>No WPC surface feature found at this point.<br>Try clicking directly on a visible front line, high, or low.<br>Location: ${sanitizeForPanel(loc)}`);
    return true;
  }
  const rows = results.slice(0, 10).map(result => {
    const attrs = result.attributes || {};
    const layerName = result.layerName || 'Surface feature';
    const label  = firstValueCI(attrs, ['name','Name','LABEL','label','featureType','FeatureType','type','TYPE','value','VALUE'], '');
    ...
    // If no real attribute value was found, label is '' — don't print "X: X".
    // Just show the layer name once.
    if (!label) return `<div class="hazard-detail-row"><span>${sanitizeForPanel(layerName)}</span>${validNote}</div>`;
    return `<div class="hazard-detail-row"><span>${sanitizeForPanel(layerName)}:</span> ${sanitizeForPanel(label)}${validNote}</div>`;
  }).join('');
  const compact = results.slice(0, 3).map(r => {
    const a = r.attributes || {};
    const layerName = r.layerName || 'Feature';
    const label = firstValueCI(a, ['name','LABEL','label','type','TYPE'], '');
    return sanitizeForPanel(label ? `${layerName}: ${label}` : layerName);
  }).join('<br>');
  setStackForce('surface', 'Surface Map Point',
    `WPC Day ${state.surfaceDay} National Forecast Chart<br>${rows}<br>Location: ${sanitizeForPanel(loc)}`,
...
async function runHrrrPoint(latlng) { if (!state.hrrrLayer || !dataEnabled('hrrr')) return false; const frame = state.hrrrFrames[state.hrrrIndex] || {}; const loc = await locationLabel(latlng.lat, latlng.lng); setStack('hrrr', 'HRRR Future Radar Point', `Current HRRR frame: <strong>${sanitizeForPanel(frame.label || `F${state.hrrrIndex}`)}</strong><br>${frame.validTime ? `Valid: ${sanitizeForPanel(frame.validTime)}<br>` : ''}Location: ${sanitizeForPanel(loc)}<br>Pixel dBZ sampling is not available from this local image overlay.`); return true; }
...
function buildExportDataHtml() {
  const types = stackOrder.filter(t => {
    const entry = state.dataStack[t];
    return Array.isArray(entry) ? entry.length > 0 : !!entry;
  });
...
async function saveMapPhoto() {
  ...
  // Leaflet-rendered <canvas> elements (e.g. county lines using L.canvas()) are
  // hidden from html2canvas's normal DOM walk and manually composited back in
  // afterward at their exact live screen position. html2canvas cannot reliably
  // snapshot a live <canvas> element in all pan/zoom states — it sometimes
  // captures it at the wrong offset. Reading the real getBoundingClientRect()
  // and drawing it ourselves is deterministic regardless of map state.
  const leafletCanvases = Array.from(document.querySelectorAll('.leaflet-overlay-pane canvas'));
  ...
  try {
    ...
    // Capture each Leaflet canvas's real on-screen rect BEFORE hiding it
    const canvasSnapshots = leafletCanvases.map(c => ({
      el: c,
      rect: c.getBoundingClientRect(),
    }));
    leafletCanvases.forEach(c => { savedVisibility.push([c, c.style.visibility]); c.style.visibility = 'hidden'; });
    ...
    // Restore visibility immediately (before any further awaits)
    savedVisibility.forEach(([c, v]) => { c.style.visibility = v; });

    // Manually composite each Leaflet canvas back onto the result at its
    // real screen position, scaled to match html2canvas's own scale factor.
    // Safety guard: only do this if exactly one canvas was found. Leaflet can
    // briefly have two canvases in the DOM during a zoom/pan animation frame
    // (old + new); compositing both at once is what caused the garbled
    // grid-like artifact seen previously. If the count looks unexpected,
    // skip the manual composite entirely rather than risk producing a worse
    // result — html2canvas's own (slightly-imperfect-but-not-garbled) default
    // capture of that canvas is the safer fallback.
    if (canvasSnapshots.length === 1) {
      const ctx = canvas.getContext('2d');
      const { el, rect } = canvasSnapshots[0];
      const onScreen = rect.width > 0 && rect.height > 0 && !el.closest('.left-stack, .data-card, .legend-card');
      if (onScreen) {
        try {
          // rect.left/top are VIEWPORT-relative; html2canvas's output represents
          // the full DOCUMENT starting at true top-left (scrollX/scrollY forced
          // to 0 in the options above) — so we must add the current scroll
          // offset to convert to the same document-relative coordinate space.
          const destX = (rect.left + window.scrollX) * scale;
          const destY = (rect.top + window.scrollY) * scale;
          ctx.drawImage(el, 0, 0, el.width, el.height,
            destX, destY, rect.width * scale, rect.height * scale);
        } catch (e) { console.warn('Could not composite canvas layer', e); }
      }
    } else if (canvasSnapshots.length > 1) {
      console.warn(`Skipped manual canvas composite — expected 1 canvas, found ${canvasSnapshots.length}.`);
    }
    ...
    console.error(error);
    savedVisibility.forEach(([c, v]) => { c.style.visibility = v; }); // safety net if the try block threw before the normal restore ran
...
// =============================================================================
// BACKEND BASE URL
// Set this to your Cloudflare R2 public URL after completing the setup guide.
// Example: 'https://pub-abc123def456.r2.dev'
// Leave empty ('') and all backend-rendered layers show as "Coming soon".
// This can also be a Netlify environment variable injected at build time.
// =============================================================================
const BACKEND_BASE_URL = 'https://pub-3a3fc1d9333043f586206edbc35017b3.r2.dev';

// CONUS bounds used by all backend-rendered image overlays
// Must match the bounds used in the Python rendering scripts (common.py)
const BACKEND_CONUS_BOUNDS = [[20, -130], [55, -60]];

// Tracks active image overlay layers created from the registry
// key: layer id (string) → value: L.imageOverlay or null
const _registryLayerStates = {};

// Toggle a registry-based image overlay layer on/off.
// Called from the checkbox onchange generated by _buildRegistryRow.
async function toggleRegistryLayer(layerId) {
  // Find the layer definition in the registry
  let layerDef = null;
  for (const cat of Object.values(LAYER_REGISTRY)) {
    const found = cat.layers?.find(l => l.id === layerId);
    if (found) { layerDef = found; break; }
  }
  if (!layerDef) return;

  // If already on, turn off
  if (_registryLayerStates[layerId]) {
    removeLayerSafe(_registryLayerStates[layerId]);
    _registryLayerStates[layerId] = null;
    updatePanel(layerDef.label, `${sanitizeForPanel(layerDef.label)} turned off.`);
    return;
  }

  // Resolve data URL — uses backend base or hardcoded url from registry
  const dataUrl = layerDef.dataUrl
    ? (BACKEND_BASE_URL ? `${BACKEND_BASE_URL}/${layerDef.dataUrl}` : null)
    : null;

  if (!dataUrl) {
    updatePanel(layerDef.label,
      `${sanitizeForPanel(layerDef.label)} is not yet available.<br>` +
      `Set BACKEND_BASE_URL in app.js after completing the GitHub Actions + R2 setup.`);
    // Uncheck the box
    const cb = document.querySelector(`input[data-registry-id="${CSS.escape(layerId)}"]`);
  ...
  const loaded = new Promise((res, rej) => {
    overlay.once('load',  () => res('ok'));
    overlay.once('error', () => rej(new Error('Image failed to load')));
  ...
  try {
    await loaded;
    _registryLayerStates[layerId] = overlay;
    updatePanel(layerDef.label,
      `<strong>${sanitizeForPanel(layerDef.label)}</strong> is on.<br>` +
      `Source: ${sanitizeForPanel(BACKEND_BASE_URL)}<br>` +
      `<small>Images refresh automatically on next GitHub Actions run.</small>`);
  } catch (e) {
    ...
    if (cb) cb.checked = false;
    updatePanel(layerDef.label,
      `Could not load ${sanitizeForPanel(layerDef.label)}.<br>` +
      `URL: ${sanitizeForPanel(dataUrl)}<br>` +
      `Check that GitHub Actions has run and R2 bucket is public.`);
  }
}
...
async function toggleGoesLayer() {
  if (state.goesLayer) {
    ...
    // Restore the basemap now that satellite imagery is off
    if (state.baseLayer) state.baseLayer.setOpacity(1);
  ...
  updateLegend('goes');
  // Hide the basemap while GOES imagery is showing. The satellite image is
  // its own complete picture of the area — leaving the basemap visible
  // underneath just causes confusing visual conflicts where the basemap's
  // labels/borders show through edges of the satellite image at a slightly
  // different geographic registration (GOES is captured from a fixed
  // satellite angle, not the same flat projection as the basemap tiles).
  if (state.baseLayer) state.baseLayer.setOpacity(0);
...
async function _loadGoesOverlay() {
  ...
  const overlay = L.imageOverlay(url, bounds, { opacity, className: 'goes-overlay', pane: 'satOverlayPane' });
...
const LAYER_REGISTRY = {
  ...
  radar: {
    ...
    layers: [
      ...
      // These registry entries document the existing layers for completeness.
      {
        ...
        status: 'active', managedBy: 'toggleRadar', decorative: true,
        sourceType: 'wms', url: 'https://opengeo.ncep.noaa.gov/geoserver/conus/conus_bref_qcd/ows',
        notes: 'Managed by existing toggleRadar() in the main Layers panel — not a separate control here.'
      },
      // Echo Tops is rendered by the GitHub Actions backend — see the dataUrl entry below.
      {
        id: 'mrmsCC', label: 'Correlation Coefficient (CC) — Dual-Pol',
        status: 'placeholder', sourceType: 'grib2-placeholder',
        dataUrl: 'mrms/cc_latest.png',
        bounds: [[20, -130], [55, -60]],
        url: 'https://mrms.ncep.noaa.gov/data/2D/CorrelationCoefficient/',
        notes: 'Rendered by GitHub Actions → R2. Set BACKEND_BASE_URL to enable.',
        dualPolExplanation: 'CC measures how similar the radar signals are in H and V polarizations. Values near 1.0 = uniform precip. Lower values = mixed-phase, large hail, or non-meteorological targets.',
      },
      {
        id: 'mrmsZDR', label: 'Differential Reflectivity (ZDR) — Dual-Pol',
        status: 'placeholder', sourceType: 'grib2-placeholder',
        dataUrl: 'mrms/zdr_latest.png',
        bounds: [[20, -130], [55, -60]],
        url: 'https://mrms.ncep.noaa.gov/data/2D/DifferentialReflectivity/',
        notes: 'Rendered by GitHub Actions → R2. Set BACKEND_BASE_URL to enable.',
        dualPolExplanation: 'ZDR = ratio of reflected power H vs V. High ZDR = large oblate raindrops. Near-zero = tumbling hail or dry snow. Negative = very large hail.',
      },
      {
        id: 'mrmsKDP', label: 'Specific Differential Phase (KDP) — Dual-Pol',
        status: 'placeholder', sourceType: 'grib2-placeholder',
        url: 'https://mrms.ncep.noaa.gov/data/2D/SpecificDifferentialPhase/',
        notes: 'Coming soon — requires server-side rendering.',
        dualPolExplanation: 'KDP = rate of change of differential phase along the beam. High KDP = heavy liquid precip. Useful for rain rate estimation, unaffected by hail.',
      },
      {
        ...
        notes: 'Coming soon — requires server-side rendering.',
        dualPolExplanation: 'HCA combines ZH, ZDR, CC, and KDP to classify precipitation type: rain, heavy rain, hail, snow, ice crystals, biological targets, etc.',
      },
      {
        id: 'mrmsEchoTops', label: 'MRMS Echo Tops (18 dBZ)',
        status: 'placeholder', sourceType: 'grib2-placeholder',
        dataUrl: 'mrms/echo_tops_latest.png',
        bounds: [[20, -130], [55, -60]],
        url: 'https://mrms.ncep.noaa.gov/data/2D/EchoTop18/',
        notes: 'Rendered by GitHub Actions → R2. Set BACKEND_BASE_URL to enable.',
      },
      {
        ...
        notes: 'Coming soon — requires server-side rendering.',
        dualPolExplanation: 'Radial velocity shows motion toward (negative) or away (positive) from the radar. Used to identify rotation, wind shear, and mesocyclones.',
      },
    ]
  },
  ...
  shortRangeModels: {
    ...
    layers: [
      // HRRR — the existing hrrrLayer is managed by toggleHrrr()
      {
        ...
        status: 'active', managedBy: 'toggleHrrr', decorative: true,
        notes: 'Managed by the existing HRRR toggle in the main Layers panel — not a separate control here.'
      },
      {
        id: 'rapCompositeRefl', label: 'RAP Composite Reflectivity',
        status: 'placeholder', sourceType: 'grib2-placeholder',
        dataUrl: 'models/rap/refl_f00.png',
        bounds: [[20, -130], [55, -60]],
      ...
      },
      {
        ...
        status: 'placeholder', sourceType: 'grib2-placeholder',
        dataUrl: 'models/rap/sfc_temp_f00.png',
        bounds: [[20, -130], [55, -60]],
        url: 'https://nomads.ncep.noaa.gov/cgi-bin/filter_rap.pl',
        notes: 'Rendered by GitHub Actions → R2. Set BACKEND_BASE_URL to enable.',
        supportsForecastHours: true
      ...
      },
      {
        ...
        status: 'placeholder', sourceType: 'grib2-placeholder',
        dataUrl: 'models/rap/cape_f00.png',
        bounds: [[20, -130], [55, -60]],
        url: 'https://nomads.ncep.noaa.gov/cgi-bin/filter_rap.pl',
        notes: 'Rendered by GitHub Actions → R2. Set BACKEND_BASE_URL to enable.',
        supportsForecastHours: true
    ...
    layers: [
      {
        ...
        status: 'placeholder', sourceType: 'grib2-placeholder',
        dataUrl: 'models/gfs/mslp_f000.png',
        bounds: [[20, -130], [55, -60]],
        url: 'https://nomads.ncep.noaa.gov/cgi-bin/filter_gfs_0p25.pl',
        notes: 'Rendered by GitHub Actions → R2. Set BACKEND_BASE_URL to enable.',
        supportsForecastHours: true
      },
      {
        ...
        status: 'placeholder', sourceType: 'grib2-placeholder',
        dataUrl: 'models/gfs/hgt500_f000.png',
        bounds: [[20, -130], [55, -60]],
        url: 'https://nomads.ncep.noaa.gov/cgi-bin/filter_gfs_0p25.pl',
        notes: 'Rendered by GitHub Actions → R2. Set BACKEND_BASE_URL to enable.',
        supportsForecastHours: true
      },
      {
        ...
        status: 'placeholder', sourceType: 'grib2-placeholder',
        dataUrl: 'models/gfs/cape_f000.png',
        bounds: [[20, -130], [55, -60]],
        url: 'https://nomads.ncep.noaa.gov/cgi-bin/filter_gfs_0p25.pl',
        notes: 'Rendered by GitHub Actions → R2. Set BACKEND_BASE_URL to enable.',
        supportsForecastHours: true
  ...
  ensembles: {
    ...
    layers: [
      {
        id: 'gefsMeanCape', label: 'GEFS Ensemble Mean CAPE',
        status: 'placeholder', sourceType: 'grib2-placeholder',
        dataUrl: 'models/gefs/mean_cape_f000.png',
        bounds: [[20, -130], [55, -60]],
        url: 'https://noaa-gefs-pds.s3.amazonaws.com/',
        notes: 'Rendered by GitHub Actions → R2 from the 21-member GEFS ensemble mean. Set BACKEND_BASE_URL to enable.',
        supportsForecastHours: true
      },
      {
        id: 'gefsMeanMslp', label: 'GEFS Ensemble Mean MSLP',
        status: 'placeholder', sourceType: 'grib2-placeholder',
        dataUrl: 'models/gefs/mean_mslp_f000.png',
        bounds: [[20, -130], [55, -60]],
        url: 'https://noaa-gefs-pds.s3.amazonaws.com/',
        notes: 'Rendered by GitHub Actions → R2 from the 21-member GEFS ensemble mean. Set BACKEND_BASE_URL to enable.',
        supportsForecastHours: true
      ...
      {
        id: 'gefsSpreadHgt500', label: 'GEFS Spread — 500 mb Height (Uncertainty)',
        status: 'placeholder', sourceType: 'grib2-placeholder',
        dataUrl: 'models/gefs/spread_hgt500_f000.png',
        bounds: [[20, -130], [55, -60]],
        url: 'https://noaa-gefs-pds.s3.amazonaws.com/',
        notes: 'Rendered by GitHub Actions → R2. Shows how much the 21 ensemble members disagree — higher spread means lower forecast confidence in that area. Set BACKEND_BASE_URL to enable.',
        supportsForecastHours: true
      },
      {
...
<div class="adv-section">
  ...
  <div class="adv-section-body" id="goesSatSection">
    ...
    <div class="adv-note">
      ...
      <br><br>Note: satellite imagery may not perfectly align with coastlines/borders.
      GOES imagery is captured from a fixed satellite angle, not a simple flat map
      projection — minor edge distortion is expected and is not a bug in this app.
    </div>
  </div>
</div>`;
}
...
function _buildRegistrySections() {
  const sectionsToShow = [
    'radar', 'upperAir', 'shortRangeModels',
  ...
  return sectionsToShow.map(catKey => {
    ...
    if (!cat) return '';
    const rows = cat.layers.filter(layer => !layer.decorative).map(layer => _buildRegistryRow(layer)).join('');
    return `
<div class="adv-section">
...
function _buildRegistryRow(layer) {
  const hasBackend   = !!layer.dataUrl;
  const backendReady = hasBackend && !!BACKEND_BASE_URL;
  const isFullyActive = layer.status === 'active';
  const isUsable = isFullyActive || layer.status === 'experimental' || backendReady;
  const disabledAttr = isUsable ? '' : 'disabled';

  // Checkbox wired to toggleRegistryLayer for any backend-ready layer
  const cbExtra = hasBackend
    ? `data-registry-id="${layer.id}" onchange="toggleRegistryLayer('${layer.id}')"`
    : '';

  // Icon: a single warning symbol for anything not fully active/stable.
  // Fully active layers (managed by existing toggles) show no icon at all.
  let icon = '';
  let detailText;
  if (isFullyActive) {
    detailText = layer.notes || 'Active layer.';
  } else if (backendReady) {
    icon = '<span class="adv-row-icon" title="Beta — backend rendering active">⚠</span>';
    detailText = layer.dualPolExplanation ||
      'Beta — rendered automatically by the GitHub Actions backend. New imagery appears on each refresh cycle.';
  } else if (layer.status === 'experimental') {
    icon = '<span class="adv-row-icon" title="Experimental — availability may vary">⚠</span>';
    detailText = layer.dualPolExplanation || layer.notes || 'Experimental — results may vary.';
  } else if (layer.status === 'disabled') {
    icon = '<span class="adv-row-icon" title="Unavailable">⚠</span>';
    detailText = 'No public source — license restriction or not publicly available.';
  } else {
    icon = '<span class="adv-row-icon" title="Coming soon">⚠</span>';
    detailText = layer.dualPolExplanation || 'Coming soon — requires server-side processing to display in browser.';
  }
  ...
  <div class="adv-row-detail collapsed" id="${rowId}">${sanitizeForPanel(detailText)}</div>
</div>`;
}

function toggleAdvRowDetail(rowId) {
  const detail = $(rowId);
  if (!detail) return;
  detail.classList.toggle('collapsed');
  const header = detail.previousElementSibling;
  const arrow = header?.querySelector('.adv-row-arrow');
  if (arrow) arrow.textContent = detail.classList.contains('collapsed') ? '▸' : '▾';
}
