...
async function saveMapPhoto() {
  ...
  const leafletCanvases = Array.from(document.querySelectorAll('.leaflet-overlay-pane canvas, .leaflet-pane canvas'));
  ...
  try {
    ...
    for (const snap of canvasSnapshots) {
      const { el, rect } = snap;
      if (rect.width <= 0 || rect.height <= 0) continue; // not visible/laid out
      // Skip if this canvas belongs to a card hidden during capture-mode
      if (el.closest('.left-stack, .data-card, .legend-card')) continue;
      try {
