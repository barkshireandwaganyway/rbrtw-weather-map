...
def find_latest_key(s3, folders):
    """
...
def process_product(s3, p):
    ...
    log.info(f'--- {name} ---')
    try:
        key, folder = find_latest_key(s3, p['folders'])
        if not key:
            raise RuntimeError(
                f'No recent .grib2.gz files found under any of: '
                f'{[f"CONUS/{f}/" for f in p["folders"]]}'
            )
